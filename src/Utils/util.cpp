/*******************************************************************
 *
 * Filename: util.cpp
 * Date:     2013-09-21
 *
 * Description: This class contains util functions
 *
 *
 *******************************************************************/


#include "util.h"
#include <math.h>
#include <QDate>
#include <QMessageBox>
#include <QTextStream>
#include <QtCore>

const CUtil::ColorRgb_ST CUtil::m_colorRgbArr[CUtil::MAX_NOF_RGB_COLORS]=
{
    {0,0,0},
    {255,0,0},
    {255,165,0},
    {255,255,0},
    {0,255,0},
    {0,255,255},
    {0,0,255},
    {255,0,255},
    {165,42,42},
    {255,192,203},
    {255,255,255},
{176, 23, 31},
    {255,140,0},
    {255,153,18},
    {0,100,0},
    {0,0,128},
    {148,0,211},
    {169,169,169},
    {255,255,224},
    {255,174,185},
    {125,158,192},
    {173,216,230},
    {224,255,255},
    {220,20,60},
    {255,182,193},
    {238,162,173},
    {205,140,149},
    {139,95,101},
    {255,181,197},
    {238,169,184},
    {205,145,158},
    {139,99,108},
    {219,112,147},
    {255,130,171},
    {238,121,159},
    {205,104,137},
    {139,71,93},
    {255,240,245},
    {238,224,229},
    {205,193,197},
    {139,131,134},
    {255,62,150},
    {238,58,140},
    {205,50,120},
    {139,34,82},
    {255,105,180},
    {255,110,180},
    {238,106,167},
    {205,96,144},
    {139,58,98},
    {135,38,87},
    {255,20,147},
    {238,18,137},
    {205,16,118},
    {139,10,80},
    {255,52,179},
    {238,48,167},
    {205,41,144},
    {139,28,98},
    {199,21,133},
    {208,32,144},
    {218,112,214},
    {255,131,250},
    {238,122,233},
    {205,105,201},
    {139,71,137},
    {216,191,216},
    {255,225,255},
    {238,210,238},
    {205,181,205},
    {139,123,139},
    {255,187,255},
    {238,174,238},
    {205,150,205},
    {139,102,139},
    {221,160,221},
    {238,130,238},
    {238,0,238},
    {205,0,205},
    {139,0,139},
    {128,0,128},
    {186,85,211},
    {224,102,255},
    {209,95,238},
    {180,82,205},
    {122,55,139},
    {153,50,204},
    {191,62,255},
    {178,58,238},
    {154,50,205},
    {104,34,139},
    {75,0,130},
    {138,43,226},
    {155,48,255},
    {145,44,238},
    {125,38,205},
    {85,26,139},
    {147,112,219},
    {171,130,255},
    {159,121,238},
    {137,104,205},
    {93,71,139},
    {72,61,139},
    {132,112,255},
    {123,104,238},
    {106,90,205},
    {131,111,255},
    {122,103,238},
    {105,89,205},
    {71,60,139},
    {248,248,255},
    {230,230,250},
    {0,0,238},
    {0,0,205},
    {0,0,139},
    {25,25,112},
    {61,89,171},
    {65,105,225},
    {72,118,255},
    {67,110,238},
    {58,95,205},
    {39,64,139},
    {100,149,237},
    {176,196,222},
    {202,225,255},
    {188,210,238},
    {162,181,205},
    {110,123,139},
    {119,136,153},
    {112,128,144},
    {198,226,255},
    {185,211,238},
    {159,182,205},
    {108,123,139},
    {30,144,255},
    {28,134,238},
    {24,116,205},
    {16,78,139},
    {240,248,255},
    {70,130,180},
    {99,184,255},
    {92,172,238},
    {79,148,205},
    {54,100,139},
    {135,206,250},
    {176,226,255},
    {164,211,238},
    {141,182,205},
    {96,123,139},
    {135,206,255},
    {126,192,238},
    {108,166,205},
    {74,112,139},
    {135,206,235},
    {0,191,255},
    {0,178,238},
    {0,154,205},
    {0,104,139},
    {51,161,201},
    {191,239,255},
    {178,223,238},
    {154,192,205},
    {104,131,139},
    {176,224,230},
    {152,245,255},
    {142,229,238},
    {122,197,205},
    {83,134,139},
    {0,245,255},
    {0,229,238},
    {0,197,205},
    {0,134,139},
    {95,158,160},
    {0,206,209},
    {240,255,255},
    {224,238,238},
    {193,205,205},
    {131,139,139},
    {209,238,238},
    {180,205,205},
    {122,139,139},
    {187,255,255},
    {174,238,238},
    {150,205,205},
    {102,139,139},
    {47,79,79},
    {151,255,255},
    {141,238,238},
    {121,205,205},
    {82,139,139},
    {0,238,238},
    {0,205,205},
    {0,139,139},
    {0,128,128},
    {72,209,204},
    {32,178,170},
    {3,168,158},
    {64,224,208},
    {128,138,135},
    {0,199,140},
    {127,255,212},
    {118,238,198},
    {102,205,170},
    {69,139,116},
    {0,250,154},
    {245,255,250},
    {0,255,127},
    {0,238,118},
    {0,205,102},
    {0,139,69},
    {60,179,113},
    {84,255,159},
    {78,238,148},
    {67,205,128},
    {46,139,87},
    {0,201,87},
    {189,252,201},
    {61,145,64},
    {240,255,240},
    {224,238,224},
    {193,205,193},
    {131,139,131},
    {143,188,143},
    {193,255,193},
    {180,238,180},
    {155,205,155},
    {105,139,105},
    {152,251,152},
    {154,255,154},
    {144,238,144},
    {124,205,124},
    {84,139,84},
    {50,205,50},
    {34,139,34},
    {0,238,0},
    {0,205,0},
    {0,139,0},
    {0,128,0},
    {48,128,20},
    {124,252,0},
    {127,255,0},
    {118,238,0},
    {102,205,0},
    {69,139,0},
    {173,255,47},
    {202,255,112},
    {188,238,104},
    {162,205,90},
    {110,139,61},
    {85,107,47},
    {107,142,35},
    {192,255,62},
    {179,238,58},
    {154,205,50},
    {105,139,34},
    {255,255,240},
    {238,238,224},
    {205,205,193},
    {139,139,131},
    {245,245,220},
    {238,238,209},
    {205,205,180},
    {139,139,122},
    {250,250,210},
    {238,238,0},
    {205,205,0},
    {139,139,0},
    {128,128,105},
    {128,128,0},
    {189,183,107},
    {255,246,143},
    {238,230,133},
    {205,198,115},
    {139,134,78},
    {240,230,140},
    {238,232,170},
    {255,250,205},
    {238,233,191},
    {205,201,165},
    {139,137,112},
    {255,236,139},
    {238,220,130},
    {205,190,112},
    {139,129,76},
    {227,207,87},
    {255,215,0},
    {238,201,0},
    {205,173,0},
    {139,117,0},
    {255,248,220},
    {238,232,205},
    {205,200,177},
    {139,136,120},
    {218,165,32},
    {255,193,37},
    {238,180,34},
    {205,155,29},
    {139,105,20},
    {184,134,11},
    {255,185,15},
    {238,173,14},
    {205,149,12},
    {139,101,8},
    {238,154,0},
    {205,133,0},
    {139,90,0},
    {255,250,240},
    {253,245,230},
    {245,222,179},
    {255,231,186},
    {238,216,174},
    {205,186,150},
    {139,126,102},
    {255,228,181},
    {255,239,213},
    {255,235,205},
    {255,222,173},
    {238,207,161},
    {205,179,139},
    {139,121,94},
    {252,230,201},
    {210,180,140},
    {156,102,31},
    {250,235,215},
    {255,239,219},
    {238,223,204},
    {205,192,176},
    {139,131,120},
    {222,184,135},
    {255,211,155},
    {238,197,145},
    {205,170,125},
    {139,115,85},
    {255,228,196},
    {238,213,183},
    {205,183,158},
    {139,125,107},
    {227,168,105},
    {237,145,33},
    {255,127,0},
    {238,118,0},
    {205,102,0},
    {139,69,0},
    {255,128,0},
    {255,165,79},
    {238,154,73},
    {205,133,63},
    {139,90,43},
    {250,240,230},
    {255,218,185},
    {238,203,173},
    {205,175,149},
    {139,119,101},
    {255,245,238},
    {238,229,222},
    {205,197,191},
    {139,134,130},
    {244,164,96},
    {199,97,20},
    {210,105,30},
    {255,127,36},
    {238,118,33},
    {205,102,29},
    {139,69,19},
    {41,36,33},
    {255,125,64},
    {255,97,3},
    {138,54,15},
    {160,82,45},
    {255,130,71},
    {238,121,66},
    {205,104,57},
    {139,71,38},
    {255,160,122},
    {238,149,114},
    {205,129,98},
    {139,87,66},
    {255,127,80},
    {255,69,0},
    {238,64,0},
    {205,55,0},
    {139,37,0},
    {94,38,18},
    {233,150,122},
    {255,140,105},
    {238,130,98},
    {205,112,84},
    {139,76,57},
    {255,114,86},
    {238,106,80},
    {205,91,69},
    {139,62,47},
    {138,51,36},
    {255,99,71},
    {238,92,66},
    {205,79,57},
    {139,54,38},
    {250,128,114},
    {255,228,225},
    {238,213,210},
    {205,183,181},
    {139,125,123},
    {255,250,250},
    {238,233,233},
    {205,201,201},
    {139,137,137},
    {188,143,143},
    {255,193,193},
    {238,180,180},
    {205,155,155},
    {139,105,105},
    {240,128,128},
    {205,92,92},
    {255,106,106},
    {238,99,99},
    {139,58,58},
    {205,85,85},
    {255,64,64},
    {238,59,59},
    {205,51,51},
    {139,35,35},
    {178,34,34},
    {255,48,48},
    {238,44,44},
    {205,38,38},
    {139,26,26},
    {238,0,0},
    {205,0,0},
    {139,0,0},
    {128,0,0},
    {142,56,142},
    {113,113,198},
    {56,142,142},
    {113,198,113},
    {142,142,56},
    {197,193,170},
    {198,113,113},
    {85,85,85},
    {30,30,30},
    {40,40,40},
    {81,81,81},
    {91,91,91},
    {132,132,132},
    {142,142,142},
    {170,170,170},
    {183,183,183},
    {193,193,193},
    {234,234,234},
    {244,244,244},
    {245,245,245},
    {220,220,220},
    {211,211,211},
    {192,192,192},
    {128,128,128},
    {105,105,105},
    {252,252,252},
    {250,250,250},
    {247,247,247},
    {245,245,245},
    {242,242,242},
    {240,240,240},
    {237,237,237},
    {235,235,235},
    {232,232,232},
    {229,229,229},
    {227,227,227},
    {224,224,224},
    {222,222,222},
    {219,219,219},
    {217,217,217},
    {214,214,214},
    {212,212,212},
    {209,209,209},
    {207,207,207},
    {204,204,204},
    {201,201,201},
    {199,199,199},
    {196,196,196},
    {194,194,194},
    {191,191,191},
    {189,189,189},
    {186,186,186},
    {184,184,184},
    {181,181,181},
    {179,179,179},
    {176,176,176},
    {173,173,173},
    {171,171,171},
    {168,168,168},
    {166,166,166},
    {163,163,163},
    {161,161,161},
    {158,158,158},
    {156,156,156},
    {153,153,153},
    {150,150,150},
    {148,148,148},
    {145,145,145},
    {143,143,143},
    {140,140,140},
    {138,138,138},
    {135,135,135},
    {133,133,133},
    {130,130,130},
    {127,127,127},
    {125,125,125},
    {122,122,122},
    {120,120,120},
    {117,117,117},
    {115,115,115},
    {112,112,112},
    {110,110,110},
    {107,107,107},
    {105,105,105},
    {102,102,102},
    {99,99,99},
    {97,97,97},
    {94,94,94},
    {92,92,92},
    {89,89,89},
    {87,87,87},
    {84,84,84},
    {82,82,82},
    {79,79,79},
    {77,77,77},
    {74,74,74},
    {71,71,71},
    {69,69,69},
    {66,66,66},
    {64,64,64},
    {61,61,61},
    {59,59,59},
    {56,56,56},
    {54,54,54},
    {51,51,51},
    {48,48,48},
    {46,46,46},
    {43,43,43},
    {41,41,41},
    {38,38,38},
    {36,36,36},
    {33,33,33},
    {31,31,31},
    {28,28,28},
    {26,26,26},
    {23,23,23},
    {20,20,20},
    {18,18,18},
    {15,15,15},
    {13,13,13},
    {10,10,10},
    {8,8,8},
    {5,5,5},
    {3,3,3}
};

#if 0
const CUtil::ColorRgb_ST CUtil::m_colorRgbArr[CUtil::MAX_NOF_RGB_COLORS]=
{
    {176, 23, 31},
        {255,192,203},
        {220,20,60},
        {216,191,216},
        {255,187,255},
        {255,0,255},
        {128,0,128},
        {138,43,226},
        {0,0,255},
        {176,196,222},
        {119,136,153},
        {240,248,255},
        {70,130,180},
        {135,206,250},
        {135,206,255},
        {135,206,235},
        {0,191,255},
        {51,161,201},
        {173,216,230},
        {176,224,230},
        {152,245,255},
        {0,245,255},
        {95,158,160},
        {0,206,209},
        {240,255,255},
        {224,255,255},
        {187,255,255},
        {47,79,79},
        {0,255,255},
        {0,128,128},
        {72,209,204},
        {32,178,170},
        {3,168,158},
        {64,224,208},
        {128,138,135},
        {0,199,140},
        {127,255,212},
        {0,250,154},
        {245,255,250},
        {0,255,127},
        {60,179,113},
        {84,255,159},
        {0,201,87},
        {189,252,201},
        {61,145,64},
        {240,255,240},
        {143,188,143},
        {152,251,152},
        {50,205,50},
        {34,139,34},
        {0,255,0},
        {0,128,0},
        {0,100,0},
        {48,128,20},
        {124,252,0},
        {127,255,0},
        {173,255,47},
        {202,255,112},
        {255,255,240},
        {245,245,220},
        {255,255,224},
        {250,250,210},
        {255,255,0},
        {128,128,105},
        {128,128,0},
        {189,183,107},
        {255,246,143},
        {238,232,170},
        {255,250,205},
        {139,129,76},
        {227,207,87},
        {255,215,0},
        {255,248,220},
        {218,165,32},
        {184,134,11},
        {255,165,0},
        {255,250,240},
        {253,245,230},
        {245,222,179},
        {255,228,181},
        {255,239,213},
        {255,235,205},
        {255,222,173},
        {252,230,201},
        {210,180,140},
        {156,102,31},
        {255,153,18},
        {250,235,215},
        {222,184,135},
        {255,228,196},
        {227,168,105},
        {237,145,33},
        {255,140,0},
        {255,128,0},
        {255,165,79},
        {250,240,230},
        {255,218,185},
        {255,245,238},
        {244,164,96},
        {199,97,20},
        {210,105,30},
        {41,36,33},
        {255,125,64},
        {255,97,3},
        {138,54,15},
        {160,82,45},
        {255,160,122},
        {255,127,80},
        {255,69,0},
        {94,38,18},
        {233,150,122},
        {255,140,105},
        {255,114,86},
        {138,51,36},
        {255,99,71},
        {250,128,114},
        {255,228,225},
        {255,250,250},
        {188,143,143},
        {240,128,128},
        {205,92,92},
        {165,42,42},
        {178,34,34},
        {255,0,0},
        {128,0,0},
        {142,56,142},
        {113,113,198},
        {125,158,192},
        {56,142,142},
        {113,198,113},
        {142,142,56},
        {197,193,170},
        {198,113,113},
        {85,85,85},
        {30,30,30},
        {170,170,170},
        {220,220,220},
        {211,211,211},
        {192,192,192},
        {169,169,169},
        {128,128,128},
        {105,105,105},
        {0,0,0}
};
#endif

#if 0
const CUtil::ColorRgb_ST CUtil::m_colorRgbArr[CUtil::MAX_NOF_RGB_COLORS]=
{
    {176, 23, 31},
    {220,20,60},
    {255,182,193},
    {255,174,185},
    {238,162,173},
    {205,140,149},
    {139,95,101},
    {255,192,203},
    {255,181,197},
    {238,169,184},
    {205,145,158},
    {139,99,108},
    {219,112,147},
    {255,130,171},
    {238,121,159},
    {205,104,137},
    {139,71,93},
    {255,240,245},
    {238,224,229},
    {205,193,197},
    {139,131,134},
    {255,62,150},
    {238,58,140},
    {205,50,120},
    {139,34,82},
    {255,105,180},
    {255,110,180},
    {238,106,167},
    {205,96,144},
    {139,58,98},
    {135,38,87},
    {255,20,147},
    {238,18,137},
    {205,16,118},
    {139,10,80},
    {255,52,179},
    {238,48,167},
    {205,41,144},
    {139,28,98},
    {199,21,133},
    {208,32,144},
    {218,112,214},
    {255,131,250},
    {238,122,233},
    {205,105,201},
    {139,71,137},
    {216,191,216},
    {255,225,255},
    {238,210,238},
    {205,181,205},
    {139,123,139},
    {255,187,255},
    {238,174,238},
    {205,150,205},
    {139,102,139},
    {221,160,221},
    {238,130,238},
    {255,0,255},
    {238,0,238},
    {205,0,205},
    {139,0,139},
    {128,0,128},
    {186,85,211},
    {224,102,255},
    {209,95,238},
    {180,82,205},
    {122,55,139},
    {148,0,211},
    {153,50,204},
    {191,62,255},
    {178,58,238},
    {154,50,205},
    {104,34,139},
    {75,0,130},
    {138,43,226},
    {155,48,255},
    {145,44,238},
    {125,38,205},
    {85,26,139},
    {147,112,219},
    {171,130,255},
    {159,121,238},
    {137,104,205},
    {93,71,139},
    {72,61,139},
    {132,112,255},
    {123,104,238},
    {106,90,205},
    {131,111,255},
    {122,103,238},
    {105,89,205},
    {71,60,139},
    {248,248,255},
    {230,230,250},
    {0,0,255},
    {0,0,238},
    {0,0,205},
    {0,0,139},
    {0,0,128},
    {25,25,112},
    {61,89,171},
    {65,105,225},
    {72,118,255},
    {67,110,238},
    {58,95,205},
    {39,64,139},
    {100,149,237},
    {176,196,222},
    {202,225,255},
    {188,210,238},
    {162,181,205},
    {110,123,139},
    {119,136,153},
    {112,128,144},
    {198,226,255},
    {185,211,238},
    {159,182,205},
    {108,123,139},
    {30,144,255},
    {28,134,238},
    {24,116,205},
    {16,78,139},
    {240,248,255},
    {70,130,180},
    {99,184,255},
    {92,172,238},
    {79,148,205},
    {54,100,139},
    {135,206,250},
    {176,226,255},
    {164,211,238},
    {141,182,205},
    {96,123,139},
    {135,206,255},
    {126,192,238},
    {108,166,205},
    {74,112,139},
    {135,206,235},
    {0,191,255},
    {0,178,238},
    {0,154,205},
    {0,104,139},
    {51,161,201},
    {173,216,230},
    {191,239,255},
    {178,223,238},
    {154,192,205},
    {104,131,139},
    {176,224,230},
    {152,245,255},
    {142,229,238},
    {122,197,205},
    {83,134,139},
    {0,245,255},
    {0,229,238},
    {0,197,205},
    {0,134,139},
    {95,158,160},
    {0,206,209},
    {240,255,255},
    {224,238,238},
    {193,205,205},
    {131,139,139},
    {224,255,255},
    {209,238,238},
    {180,205,205},
    {122,139,139},
    {187,255,255},
    {174,238,238},
    {150,205,205},
    {102,139,139},
    {47,79,79},
    {151,255,255},
    {141,238,238},
    {121,205,205},
    {82,139,139},
    {0,255,255},
    {0,238,238},
    {0,205,205},
    {0,139,139},
    {0,128,128},
    {72,209,204},
    {32,178,170},
    {3,168,158},
    {64,224,208},
    {128,138,135},
    {0,199,140},
    {127,255,212},
    {118,238,198},
    {102,205,170},
    {69,139,116},
    {0,250,154},
    {245,255,250},
    {0,255,127},
    {0,238,118},
    {0,205,102},
    {0,139,69},
    {60,179,113},
    {84,255,159},
    {78,238,148},
    {67,205,128},
    {46,139,87},
    {0,201,87},
    {189,252,201},
    {61,145,64},
    {240,255,240},
    {224,238,224},
    {193,205,193},
    {131,139,131},
    {143,188,143},
    {193,255,193},
    {180,238,180},
    {155,205,155},
    {105,139,105},
    {152,251,152},
    {154,255,154},
    {144,238,144},
    {124,205,124},
    {84,139,84},
    {50,205,50},
    {34,139,34},
    {0,255,0},
    {0,238,0},
    {0,205,0},
    {0,139,0},
    {0,128,0},
    {0,100,0},
    {48,128,20},
    {124,252,0},
    {127,255,0},
    {118,238,0},
    {102,205,0},
    {69,139,0},
    {173,255,47},
    {202,255,112},
    {188,238,104},
    {162,205,90},
    {110,139,61},
    {85,107,47},
    {107,142,35},
    {192,255,62},
    {179,238,58},
    {154,205,50},
    {105,139,34},
    {255,255,240},
    {238,238,224},
    {205,205,193},
    {139,139,131},
    {245,245,220},
    {255,255,224},
    {238,238,209},
    {205,205,180},
    {139,139,122},
    {250,250,210},
    {255,255,0},
    {238,238,0},
    {205,205,0},
    {139,139,0},
    {128,128,105},
    {128,128,0},
    {189,183,107},
    {255,246,143},
    {238,230,133},
    {205,198,115},
    {139,134,78},
    {240,230,140},
    {238,232,170},
    {255,250,205},
    {238,233,191},
    {205,201,165},
    {139,137,112},
    {255,236,139},
    {238,220,130},
    {205,190,112},
    {139,129,76},
    {227,207,87},
    {255,215,0},
    {238,201,0},
    {205,173,0},
    {139,117,0},
    {255,248,220},
    {238,232,205},
    {205,200,177},
    {139,136,120},
    {218,165,32},
    {255,193,37},
    {238,180,34},
    {205,155,29},
    {139,105,20},
    {184,134,11},
    {255,185,15},
    {238,173,14},
    {205,149,12},
    {139,101,8},
    {255,165,0},
    {238,154,0},
    {205,133,0},
    {139,90,0},
    {255,250,240},
    {253,245,230},
    {245,222,179},
    {255,231,186},
    {238,216,174},
    {205,186,150},
    {139,126,102},
    {255,228,181},
    {255,239,213},
    {255,235,205},
    {255,222,173},
    {238,207,161},
    {205,179,139},
    {139,121,94},
    {252,230,201},
    {210,180,140},
    {156,102,31},
    {255,153,18},
    {250,235,215},
    {255,239,219},
    {238,223,204},
    {205,192,176},
    {139,131,120},
    {222,184,135},
    {255,211,155},
    {238,197,145},
    {205,170,125},
    {139,115,85},
    {255,228,196},
    {238,213,183},
    {205,183,158},
    {139,125,107},
    {227,168,105},
    {237,145,33},
    {255,140,0},
    {255,127,0},
    {238,118,0},
    {205,102,0},
    {139,69,0},
    {255,128,0},
    {255,165,79},
    {238,154,73},
    {205,133,63},
    {139,90,43},
    {250,240,230},
    {255,218,185},
    {238,203,173},
    {205,175,149},
    {139,119,101},
    {255,245,238},
    {238,229,222},
    {205,197,191},
    {139,134,130},
    {244,164,96},
    {199,97,20},
    {210,105,30},
    {255,127,36},
    {238,118,33},
    {205,102,29},
    {139,69,19},
    {41,36,33},
    {255,125,64},
    {255,97,3},
    {138,54,15},
    {160,82,45},
    {255,130,71},
    {238,121,66},
    {205,104,57},
    {139,71,38},
    {255,160,122},
    {238,149,114},
    {205,129,98},
    {139,87,66},
    {255,127,80},
    {255,69,0},
    {238,64,0},
    {205,55,0},
    {139,37,0},
    {94,38,18},
    {233,150,122},
    {255,140,105},
    {238,130,98},
    {205,112,84},
    {139,76,57},
    {255,114,86},
    {238,106,80},
    {205,91,69},
    {139,62,47},
    {138,51,36},
    {255,99,71},
    {238,92,66},
    {205,79,57},
    {139,54,38},
    {250,128,114},
    {255,228,225},
    {238,213,210},
    {205,183,181},
    {139,125,123},
    {255,250,250},
    {238,233,233},
    {205,201,201},
    {139,137,137},
    {188,143,143},
    {255,193,193},
    {238,180,180},
    {205,155,155},
    {139,105,105},
    {240,128,128},
    {205,92,92},
    {255,106,106},
    {238,99,99},
    {139,58,58},
    {205,85,85},
    {165,42,42},
    {255,64,64},
    {238,59,59},
    {205,51,51},
    {139,35,35},
    {178,34,34},
    {255,48,48},
    {238,44,44},
    {205,38,38},
    {139,26,26},
    {255,0,0},
    {238,0,0},
    {205,0,0},
    {139,0,0},
    {128,0,0},
    {142,56,142},
    {113,113,198},
    {125,158,192},
    {56,142,142},
    {113,198,113},
    {142,142,56},
    {197,193,170},
    {198,113,113},
    {85,85,85},
    {30,30,30},
    {40,40,40},
    {81,81,81},
    {91,91,91},
    {132,132,132},
    {142,142,142},
    {170,170,170},
    {183,183,183},
    {193,193,193},
    {234,234,234},
    {244,244,244},
    {255,255,255},
    {245,245,245},
    {220,220,220},
    {211,211,211},
    {192,192,192},
    {169,169,169},
    {128,128,128},
    {105,105,105},
    {0,0,0},
    {252,252,252},
    {250,250,250},
    {247,247,247},
    {245,245,245},
    {242,242,242},
    {240,240,240},
    {237,237,237},
    {235,235,235},
    {232,232,232},
    {229,229,229},
    {227,227,227},
    {224,224,224},
    {222,222,222},
    {219,219,219},
    {217,217,217},
    {214,214,214},
    {212,212,212},
    {209,209,209},
    {207,207,207},
    {204,204,204},
    {201,201,201},
    {199,199,199},
    {196,196,196},
    {194,194,194},
    {191,191,191},
    {189,189,189},
    {186,186,186},
    {184,184,184},
    {181,181,181},
    {179,179,179},
    {176,176,176},
    {173,173,173},
    {171,171,171},
    {168,168,168},
    {166,166,166},
    {163,163,163},
    {161,161,161},
    {158,158,158},
    {156,156,156},
    {153,153,153},
    {150,150,150},
    {148,148,148},
    {145,145,145},
    {143,143,143},
    {140,140,140},
    {138,138,138},
    {135,135,135},
    {133,133,133},
    {130,130,130},
    {127,127,127},
    {125,125,125},
    {122,122,122},
    {120,120,120},
    {117,117,117},
    {115,115,115},
    {112,112,112},
    {110,110,110},
    {107,107,107},
    {105,105,105},
    {102,102,102},
    {99,99,99},
    {97,97,97},
    {94,94,94},
    {92,92,92},
    {89,89,89},
    {87,87,87},
    {84,84,84},
    {82,82,82},
    {79,79,79},
    {77,77,77},
    {74,74,74},
    {71,71,71},
    {69,69,69},
    {66,66,66},
    {64,64,64},
    {61,61,61},
    {59,59,59},
    {56,56,56},
    {54,54,54},
    {51,51,51},
    {48,48,48},
    {46,46,46},
    {43,43,43},
    {41,41,41},
    {38,38,38},
    {36,36,36},
    {33,33,33},
    {31,31,31},
    {28,28,28},
    {26,26,26},
    {23,23,23},
    {20,20,20},
    {18,18,18},
    {15,15,15},
    {13,13,13},
    {10,10,10},
    {8,8,8},
    {5,5,5},
    {3,3,3}

};
#endif





/*********************************************************************
 * Function: CUtil()
 *
 * Description:
 *
 *********************************************************************/
CUtil::CUtil()
{

}


/*********************************************************************
 * Function: CUtil()
 *
 * Description:
 *
 *********************************************************************/
bool CUtil::saveTextFile(QString dataToStore, QString filename, QString path)
{
    QString outputFilename;
    outputFilename = path;
    outputFilename += "/";
    outputFilename += filename;


    QFile outputFile(outputFilename);
    outputFile.open(QIODevice::WriteOnly);

    if(!outputFile.isOpen())
    {
        qDebug()  << "Failed to open file" << outputFilename;
        return false;
    }

    // Point a QTextStream object at the file
    QTextStream outStream(&outputFile);

   //* Write data to the file
   outStream << dataToStore;

   outputFile.close();
}



/*********************************************************************
 * Function: setColor()
 *
 * Description: Change line color in graph.
 *
 *********************************************************************/
QColor CUtil::getQColor(CUtil::ColorRgb_ET color)
{
    if(color >= CUtil::MAX_NOF_RGB_COLORS)
        color = (CUtil::ColorRgb_ET)(CUtil::MAX_NOF_RGB_COLORS - ((CUtil::ColorRgb_ET) 1));

    return QColor::fromRgb(m_colorRgbArr[color].r, m_colorRgbArr[color].g, m_colorRgbArr[color].b);
}

/*********************************************************************
 * Function: setColor()
 *
 * Description: Change line color in graph.
 *
 *********************************************************************/
QPen CUtil::setColor(CUtil::ColorRgb_ET color)
{

    QPen linePen(QColor::fromRgb(m_colorRgbArr[color].r, m_colorRgbArr[color].g, m_colorRgbArr[color].b));

    return linePen;

}

/*********************************************************************
 * Function: calcMeanAndStdDeviation()
 *
 * Description: Calculates mean and standadard deviation.
 *
 *********************************************************************/
bool CUtil::calcMeanAndStdDeviation(double inputData[],
                               int nofData,
                               double &mean,
                               double &stdDev)
{
    int i;
    mean = 0;
    stdDev = 0;

    if(nofData < 0)
    {
        return false;
    }
    else if(nofData == 0)
    {
        return true;
    }


    if(false == calcMean(inputData, nofData, mean))
    {
        return false;
    }


    for(i = 0; i < nofData; i++)
    {
        stdDev += pow((inputData[i] - mean), 2);
    }

    stdDev = stdDev / nofData;
    stdDev = sqrt(stdDev);
    return true;

}



/*********************************************************************
 * Function: calcMean()
 *
 * Description: Calculates mean value.
 *
 *********************************************************************/
bool CUtil::calcMean(double inputData[],
                     int nofData,
                     double &mean)
{
    int i;
    mean = 0;

    if(nofData < 0)
    {
        return false;
    }
    else if(nofData == 0)
    {
        return true;
    }


    for(i = 0; i < nofData; i++)
    {
        mean += inputData[i];
    }
    mean = mean / nofData;
    return true;

}


/*********************************************************************
 * Function: number2Int()
 *
 * Description: Convert qstring to int.
 *
 *********************************************************************/
bool CUtil::number2Int(QString number, int &intNumber)
{
    int i;
    int len;

    if(number.isNull() == true)
        return false;

    len = number.length();

    for(i = 0; i < len; i++)
    {
        if(!((number.at(i)>= '0') &&  (number.at(i)<= '9')))
            return false;
    }

    intNumber = number.toInt();

    return true;
}



/*********************************************************************
 * Function: number2DoubleRemoveSpace()
 *
 * Description: Convert qstring to double.
 *
 *********************************************************************/
bool CUtil::number2DoubleRemoveSpace(QString number, double &dbNumber)
{
    int i;
    int len;
    int init;

    if(number.isNull() == true)
        return false;

    if(number.size() == 0)
        return false;


  // Remove whitespaces inside a string
   number = number.replace(" ","");



    len = number.length();

    if(number[0]== '-')
        init = 1;
    else
        init = 0;

    for(i = init; i < len; i++)
    {
        if(!((number.at(i)>= '0') && (number.at(i)<= '9')))
        {
            if(number.at(i)!= ',')
                return false;
        }

    }

    dbNumber = number.toDouble();

    return true;
}



/*********************************************************************
 * Function: number2double()
 *
 * Description: Convert qstring to int.
 *
 *********************************************************************/
bool CUtil::number2double(QString number, double &doubleNumber)
{
    int i;
    int len;

    if(number.isNull() == true)
        return false;

    number.replace(',','.');

    len = number.length();

    for(i = 0; i < len; i++)
    {
        if(!(((number.at(i)>= '0') && (number.at(i)<= '9')) || (number.at(i)== '.') || number.at(0) == '-'))
            return false;
    }

    doubleNumber = number.toDouble();

    return true;
}



/*********************************************************************
 * Function: number2double()
 *
 * Description: Convert qstring to int.
 *
 *********************************************************************/
bool CUtil::number2double(QString number, QString &doubleNumber)
{
    int i;
    int len;

    if(number.isNull() == true)
        return false;

    number.replace(',','.');

    len = number.length();

    for(i = 0; i < len; i++)
    {
        if(!(((number.at(i)>= '0') && (number.at(i)<= '9')) || (number.at(i)== '.') || number.at(0) == '-'))
            return false;
    }

    doubleNumber = number;

    return true;
}

/*********************************************************************
 * Function: createFileDriectory()
 *
 * Description:
 *
 *
 *********************************************************************/
bool CUtil::createFileDriectory(QString pathAndMapname)
{

    // Create sockname directory
    if(QDir(pathAndMapname).exists() == false)
    {
        if(QDir().mkdir(pathAndMapname) == false)
        {
            //QMessageBox::information(NULL,
            //                         QString::fromUtf8("Error"),
            //                         QString::fromUtf8("Fail to create directory"));
            return false;
        }
    }
    return true;
}

/*********************************************************************
 * Function: createProperFilname()
 *
 * Description: The filname are using following chaacters:
 *
 *              A - Z
 *              a - z
 *              0 - 9
 *              '-',      minus
 *              '_',      undersore
 *              '.'       dot 4 char from end.
 *
 *********************************************************************/
bool CUtil::createProperFilname(QString inFilename, QString &outFilename, bool checkExeDot)
{
    int extentionPos = (inFilename.length() - 4);

    // Filname need to be 5 chars (y.xxx)
    if(extentionPos < 1)
    {
        return false;
    }

    QString tmp = ".";
    // check if file contains .xxx in the end
    if((checkExeDot == true) && (inFilename[extentionPos] != tmp[0]))
    {
        return false;
    }

    // Only add valid charachters
    for(int i = 0, j = 0; i < inFilename.length(); i++)
    {
        if((inFilename.at(i)) >= 'a' && (inFilename.at(i) <= 'z'))
        {
            outFilename.insert(j++, inFilename[i]);
        }
        else if((inFilename.at(i)) >= 'A' && (inFilename.at(i) <= 'Z'))
        {
            outFilename.insert(j++, inFilename[i]);
        }
        else if((inFilename.at(i)) >= '0' && (inFilename.at(i) <= '9'))
        {
            outFilename.insert(j++, inFilename[i]);
        }
        else if((inFilename.at(i) == '.') && (inFilename.length() - 4 == i))
        {
            outFilename.insert(j++, inFilename[i]);
        }
        else if((inFilename.at(i)) == '-' || (inFilename.at(i) == '_'))
        {
            outFilename.insert(j++, inFilename[i]);
        }
        else if((inFilename.at(i)) == ' ')
        {
            QString tmp;
                    tmp.clear();
                    tmp = "_";
            outFilename.insert(j++, tmp);
        }
    }

    if(outFilename.length() > 2)
    {
        return true;
    }

    return false;
}




/*********************************************************************
 * Function: dateIsValid()
 *
 * Description: yyyy-mm-dd.
 *
 *********************************************************************/
bool CUtil::dateIsValid(QString date)
{
    int i;
    QString tmpChar;


    if(date.length() < 10)
        return false;

    if(date.at(4) != '-')
        return false;

    if(date.at(7) != '-')
        return false;

    for(i = 0; i < date.length(); i++)
    {
       // tmpChar.sprintf("%d, %c", date.at(i), date.at(i));
       // qDebug() << "tmpchar" << tmpChar;
        if( !((date.at(i)>= '0' && date.at(i) <= '9') || date.at(i) == '-') /*|| date.at(date.length()-1) == 10*/)
        {
            return false;
        }
    }

    return true;
}


/*********************************************************************
 * Function: getCurrentDate()
 *
 * Description: return current date on the format yyyy-mm-dd.
 *
 *********************************************************************/
void CUtil::getCurrentDate(QString &dateStr)
{
    QDate date = date.currentDate();
    dateStr = date.toString("yyyy-MM-dd");

}


/*********************************************************************
 * Function: splitDate()
 *
 * Description: .
 *
 *********************************************************************/
bool CUtil::splitDate(QString date, QString &year, QString &month, QString &day)
{
    if(false == dateIsValid(date))
        return false;

    year = date.mid(0,4);
    month = date.mid(5,2);
    day = date.mid(8,2);

    return true;
}





/*********************************************************************
 * Function: addDays()
 *
 * Description: .
 *
 *********************************************************************/
bool CUtil::addDays(QString inputDate, QString &outputDate, int day)
{
    QDate myDate;
    QString strYear;
    QString strMonth;
    QString strDay;

    if(false == dateIsValid(inputDate))
        return false;

    splitDate(inputDate, strYear, strMonth, strDay);

    myDate.setDate(strYear.toInt(), strMonth.toInt(), strDay.toInt());

    myDate = myDate.addDays(day);
    outputDate.sprintf("%.4d-%.2d-%.2d", myDate.year(), myDate.month(), myDate.day());

    return true;
}


/*********************************************************************
 * Function: addMonth()
 *
 * Description: .
 *
 *********************************************************************/
bool CUtil::addMonth(QString inputDate, QString &outputDate, int intMonth)
{
    QDate myDate;
    QString strYear;
    QString strMonth;
    QString strDay;

    if(false == dateIsValid(inputDate))
        return false;

    splitDate(inputDate, strYear, strMonth, strDay);

    myDate.setDate(strYear.toInt(), strMonth.toInt(), strDay.toInt());

    myDate = myDate.addMonths(intMonth);
    outputDate.sprintf("%.4d-%.2d-%.2d", myDate.year(), myDate.month(), myDate.day());

    return true;
}

/*********************************************************************
 * Function: addYear()
 *
 * Description: .
 *
 *
 *
 *
 *********************************************************************/
bool CUtil::addYear(QString inputDate, QString &outputDate, int intYear)
{
    QDate myDate;
    QString strYear;
    QString strMonth;
    QString strDay;

    if(false == dateIsValid(inputDate))
        return false;

    splitDate(inputDate, strYear, strMonth, strDay);

    myDate.setDate(strYear.toInt(), strMonth.toInt(), strDay.toInt());

    myDate = myDate.addYears(intYear);
    outputDate.sprintf("%.4d-%.2d-%.2d", myDate.year(), myDate.month(), myDate.day());

    qDebug() << "Year outputDate" << outputDate;
    return true;
}


/*********************************************************************
 * Function: addYear()
 *
 * Description: .
 *
 *
 *
 *
 *********************************************************************/
bool CUtil::getLinuxTime(QString inputDate, uint &unixtimestamp)
{
    QDate myDate;
    QDateTime myDateTime;
    QString strYear;
    QString strMonth;
    QString strDay;

    if(false == dateIsValid(inputDate))
        return false;

    splitDate(inputDate, strYear, strMonth, strDay);

    myDate.setDate(strYear.toInt(), strMonth.toInt(), strDay.toInt());
    myDateTime.setDate(myDate);
    unixtimestamp = myDateTime.toTime_t();

    qDebug() << "unixtimestamp" << unixtimestamp;
    return true;
}



/*********************************************************************
 * Function:    calcDeltaMonth()
 *
 * Description: .
 *
 *
 *
 *
 *********************************************************************/
int CUtil::calcDeltaMonth(int year, int month, int minYear, int minMonth)
{
    int maxMonths = year * 12 + month;
    int minMonths = minYear * 12 + minMonth;
    int resMonth = maxMonths - minMonths;

   return resMonth;
}


/*********************************************************************
 * Function: nofDaysBeteenDates()
 *
 * Description: .
 *
 * Note:    QDate d1(1995, 5, 17);  // May 17, 1995
 *          QDate d2(1995, 5, 20);  // May 20, 1995
 *
 *          d1.daysTo(d2);          // returns 3
 *          d2.daysTo(d1);          // returns -3
 *
 *
 *
 *
 *********************************************************************/
bool CUtil::nofDaysBeteenDates(QString date1, QString date2, int &nofdays)
{
    QDate myDate1;
    QDate myDate2;
    QString strYear;
    QString strMonth;
    QString strDay;

    if(false == dateIsValid(date1))
        return false;

    if(false == dateIsValid(date2))
        return false;

    splitDate(date1, strYear, strMonth, strDay);
    myDate1.setDate(strYear.toInt(), strMonth.toInt(), strDay.toInt());

    splitDate(date2, strYear, strMonth, strDay);
    myDate2.setDate(strYear.toInt(), strMonth.toInt(), strDay.toInt());


    nofdays = myDate1.daysTo(myDate2);

    qDebug() << "Nofdays" << nofdays;
    return true;
}



/*********************************************************************
 * Function: createArrMemSpace()
 *
 * Description: .
 *
 *********************************************************************/
bool CUtil::createArrMemSpace(int nofStocks, double **arr)
{

    // Create variances covariances matrix space
    (*arr) = new double [nofStocks + 5];

    if((*arr) == 0)
    {
        return false;
    }
    return true;
}


/*********************************************************************
 * Function: createMatrixMemSpace()
 *
 * Description: .
 *
 *********************************************************************/
bool CUtil::createMatrixMemSpace(int nofStocks, double ***matrix)
{
    int i;

    (*matrix) = new double *[nofStocks + 5];

    if((*matrix) == 0)
    {
        return false;
    }

    for(i = 0; i < nofStocks; i++)
    {
        (*matrix)[i] = new double[nofStocks + 5];
        if((*matrix)[i] == 0)
        {
            return false;
        }
    }

    return true;
}



/*********************************************************************
 * Function: ()
 *
 * Description: .
 *
 *********************************************************************/
void CUtil::removeMatrixMemSpace(int nofStocks, double ***matrix)
{
    int i;

    for(i = 0; i < nofStocks; i++)
    {
        delete (*matrix)[i];
    }

    delete (*matrix);
}


/*********************************************************************
 * Function: ()
 *
 * Description: .
 *
 *********************************************************************/
void CUtil::deleteArrMemSpace(double **arr)
{
    delete (*arr);
}

/*********************************************************************
 * Function: ()
 *
 * Description: .
 *
 *********************************************************************/
void CUtil::htmlCodesToChar(QString inStr, QString &outStr)
{
    outStr = inStr.replace(QString::fromUtf8("&#246;"), QString::fromUtf8("ö"));
    outStr = outStr.replace(QString::fromUtf8("&#228;"), QString::fromUtf8("ä"));
    outStr = outStr.replace(QString::fromUtf8("&#229;"), QString::fromUtf8("å"));

    outStr = outStr.replace(QString::fromUtf8("&#214;"), QString::fromUtf8("Ö"));
    outStr = outStr.replace(QString::fromUtf8("&#196;"), QString::fromUtf8("Ä"));
    outStr = outStr.replace(QString::fromUtf8("&#197;"), QString::fromUtf8("Å"));
    outStr = outStr.replace(QString::fromUtf8("&#233;"), QString::fromUtf8("é"));
    outStr = outStr.replace(QString::fromUtf8("&#248;"), QString::fromUtf8("ø"));
    outStr = outStr.replace(QString::fromUtf8("'"), QString::fromUtf8("\''"));
    outStr = outStr.replace(QString::fromUtf8("&amp;"), QString::fromUtf8("&"));

}




/*********************************************************************
 * Function: ()
 *
 * Description: .
 *
 *********************************************************************/
bool CUtil::annualGrowthRate(double startValue, double endValue, double nofYears, double &growthRate)
{
    if(startValue == 0 || nofYears == 0)
        return false;

    growthRate = pow((endValue/startValue), (1/nofYears));
    return true;
}


/*******************************************************************
 *
 * Function:    createStartEndDates()
 *
 * Description: Get current date (end date) on format yyyy-mm-yy and
 *              calculate start date by remove x nof month.
 *
 * Note:        nofMonthInBetween: Need to be negative
 *
 *******************************************************************/
bool CUtil::createStartEndDates(QString &startDate,
                                QString &endDate,
                                int nofMonthInBetween)
{
    bool res = true;

    if(nofMonthInBetween >= 0)
    {
        QMessageBox::information(NULL, QString::fromUtf8("Create Dates"), QString::fromUtf8("Fail: nofMonthInBetween need to be < 0"));
        res = false;
    }
    else
    {
        getCurrentDate(endDate);

        if(false == addMonth(endDate, startDate, nofMonthInBetween))
        {
            res = false;
        }
    }

    return res;
}

