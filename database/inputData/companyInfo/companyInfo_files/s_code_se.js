...
s.linkInternalFilters="javascript:,nordnet.se,nordnet.lu";
s.linkLeaveQueryString=false;
s.linkTrackVars="None";
s.linkTrackEvents="None";
s.prop6="$Revision: 179 $";

/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="nordnet";
s.trackingServer="w320.nordnet.se";
s.trackingServerSecure="w319.nordnet.se";
...
function s_doPlugins(s) {
	...
	if(s.campaign)
		s.campaign=s.getValOnce(s.campaign,'cmpe',0);
	
	var kwcid=s.getQueryParam('s_kwcid');
		kwcid=s.getValOnce(kwcid,'kwcid',0);
	
	if (kwcid) {
		var kwcids = kwcid.split('|');
		if (kwcids.length >= 3)
			s.eVar6=s.crossVisitParticipation(kwcids[2], 'evar6', 30, 5, ' > ');
	}
	...
	if(s.campaign) {
		var swo_cvchannels = {ppc:"Paid search",ban:"Display Ad",aff:"Affiliate Ad",social:"Social Media",par:"Partners",mail:"Email",pjs:"Price Comparison"};
		var swo_cvkeys = s.campaign.toLowerCase().split('-');
		var swo_cvkey = ( swo_cvkeys[0] == 'ban' && swo_cvkeys[1] == 'aff' ? swo_cvkeys[1] : swo_cvkeys[0] );
		
		if( ! swo_cvchannels[swo_cvkey] )
			swo_cvchannel = 'Undefined';
		else
			swo_cvchannel = swo_cvchannels[swo_cvkey];
		
		s.eVar4 = s.crossVisitParticipation(swo_cvchannel, 'evar4', 30, 5, ' > ');
		s.eVar5 = s.crossVisitParticipation(s.campaign, 'evar5', 30, 5, ' > ');
	}
	
	if (!s.campaign && !kwcid) {
		var cvref = document.referrer.toLowerCase();
		if (cvref && cvref !== '') {
			if (cvref.indexOf('google') !== -1 || cvref.indexOf('yahoo') !== -1 || cvref.indexOf('bing') !== -1)
				s.eVar4 = s.crossVisitParticipation('Organic search', 'evar4', 30, 5, ' > ');
		}
	}
}
...
s.Integrate.Eyeblaster_ACM.aID=63116
s.Integrate.Eyeblaster_ACM.cID=162628
s.Integrate.Eyeblaster_ACM.tVar="eVar10"
...
if(((at&&a1&&a2&&a3)||(bt&&b1&&b2&&b3))&&!p.errorCode)s[p.tVar]="EB_ACM:"+(at?at:0)+":"+(a1?a1:0)+":"+(a2?a2:0)+":"+(a3?a3:0)+":"+(bt?bt:0)+":"+(b1?b1:0)+":"+(b2?b2:0)+":"+(b3?b3:0)
	}
}

/*
 * Plug-in: crossVisitParticipation(eVarN,cookie_name,cookielife,numberoflevels,delimiter) v1.7 - stacks values from
 * specified variable in cookie and returns value
 *
 * Param: eVarN: eVar to be tracked across visits
 *        cookie_name: String, cookie name for storing
 *        cookielife: String, time when cookie expires in number of days (>0)
 *        numberoflevels: number of levels that include in return seperated by delimiters
 *        delimiter: char used to seperate levels
 *
 * NOTE: crossVisitParticipation must be used with repl(),split() and join().
 */
